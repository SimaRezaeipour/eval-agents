...
def complete(system: str, user: str, max_tokens: int = 2048) -> str:
    ...
    Automatically selects Anthropic or Azure OpenAI based on which key is configured.
    Both backends are instructed to return JSON only.
    """
    settings = get_settings()

    if settings.anthropic_api_key.get_secret_value():
        return _anthropic_complete(system, user, max_tokens, settings)
    elif settings.azure_openai_key.get_secret_value():
        return _azure_complete(system, user, max_tokens, settings)
    else:
        raise RuntimeError(
            "No LLM credentials found. Set ANTHROPIC_API_KEY in .env "
            "(or AZURE_OPENAI_KEY + AZURE_OPENAI_ENDPOINT for Azure OpenAI)."
        )
